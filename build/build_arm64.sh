#!/bin/bash
# Apple Silicon Mac构建脚本

set -e

echo "开始构建Apple Silicon版本..."

# 检查是否在ARM64架构上运行
if [ "$(uname -m)" != "arm64" ]; then
    echo "警告: 当前不在Apple Silicon Mac上，可能无法正确构建ARM64版本"
fi
...
cd build
pyinstaller --clean etf_caller.spec --target-arch=arm64
...
if [ -f "dist/etf_api_caller" ]; then
    ...
    # 复制到项目根目录
    cp dist/etf_api_caller ../etf_api_caller_arm64
    echo "已复制到: ../etf_api_caller_arm64"
else
    echo "❌ 构建失败"
    exit 1
fi
